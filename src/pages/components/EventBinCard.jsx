...
const EventBinCard = ({ index, event, removeEvent }) => {
  ...
  const handleRestore = async () => {
    const result = await postRequest("recycle/restoreEvent", {
      uuid: event.uuid,
    });
  ...
  const handleDelete = async () => {
    const result = await deleteRequest("recycle/deleteEvent", {
      uuid: event.uuid,
    });
    ...
    >
      <header className="flex justify-between items-center">
        ...
        <h2 className="text-xs italic">
          {" "}
          Deleted {useEventDuration(
            event.recycle_datetime,
            new Date()
          )} ago{" "}
        </h2>
      </header>
      <div className="flex flex-col gap-1 text-sm">
        {event.location && <h2> Location: {event.location} </h2>}
        <h2> Created: {useDateFormat(event.created_datetime)} </h2>
        <h2> {useEventCountDown(event.start_datetime)} </h2>
        <h2>
          {" "}
          Duration: {useEventDuration(
